...
Нейронная сеть для распознавания цифр 0–9 на чёрно-белых BMP-изображениях 16×16. Реализация на C++ без внешних библиотек: свёртка, max-pooling, полносвязные слои, ReLU, softmax, обучение методом обратного распространения.
...
```bash
./build/neural_network [путь_к_данным] [путь_к_cores] [путь_к_biases]
```
...
При обучении программа спросит число эпох (по умолчанию 3).
...
- **Слой 1:** свёрточный, 16 карт признаков, ядро 2×2, ReLU
